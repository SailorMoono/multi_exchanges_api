...
class Bitget extends ExchangeBase {
  ...
  async baseGet(URI,pathData){
    ...
    let res = null
    try {
        res = await this.get(URL,null, {headers: params.headers})
    } catch (error) {
        res = error.response;
        // console.log(error)
    }
  ...
  encrypt(crypData, secretKey) {
    if (!crypData || typeof crypData !== 'string') {
      throw new Error(`Invalid crypData: ${crypData}`);
    }
    if (!secretKey || typeof secretKey !== 'string') {
      throw new Error(`Invalid secretKey: ${secretKey}`);
    }

    console.log('Encrypting data:', crypData);
